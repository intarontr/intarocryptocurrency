...
TMP_FOLDER=$(mktemp -d)
CONFIG_FILE='Intarocryptocurrency.conf'
CONFIG_FOLDER='"$HOME"/.Intarocryptocurrency'
BACKUP_FOLDER="$HOME/IntarocryptocurrencyBackups"
COIN_DAEMON='intarocryptocurrencyd'
COIN_PATH='/usr/bin/'
COIN_REPO='https://github.com/intarocryptocurrency/intarocryptocurrency.git'
#COIN_TGZ='http://github.com/intarocryptocurrency/intarocryptocurrency/releases/XXX.zip'
#COIN_ZIP=$(echo $COIN_TGZ | awk -F'/' '{print $NF}')
COIN_NAME='Intarocryptocurrency'
COIN_PORT=44144
RPC_PORT=44155
...
purgeOldInstallation() {
    ...
    test -d ~/IntarocryptocurrencyBackups && echo "Backups folder exists, skipping directory creation..." || mkdir ~/IntarocryptocurrencyBackups
    iteration=0
    ...
    mv $CONFIG_FOLDER/Intarocryptocurrency.conf $BACKUP_FOLDER/$foldername
...
function update_config() {
  ...
  echo -e "${BLUE}================================================================================================================================${NC}"
  echo -e "${PURPLE}Please follow this guide to setup the controller wallet, then return here to input your the genkey output: https://github.com/intarocryptocurrency/intarocryptocurrency/wiki/Setup-Manager---Masternode-Asisstant-Setup-Script-Guide${NC}"
  echo -e "${BLUE}================================================================================================================================${NC}"
  read -p "${PURPLE}Please enter the ${NC}${GREEN}genkey${NC}:" COINKEY
  systemctl stop intarocryptocurrency
...
EOF
systemctl start intarocryptocurrency
}
...
function prepare_system() {
    if [ -f ./install-dependencies.sh ]; then
        ...
        wget https://raw.githubusercontent.com/intarocryptocurrency/intarocryptocurrency/master/scripts/install-dependencies.sh
        chmod +x install-dependencies.sh
        ./install-dependencies.sh
    fi
    if [ "$?" -gt "0" ];
...
function important_information() {
    ...
    echo -e "${BLUE}================================================================================================================================"
    echo -e "${CYAN}Follow twitter to stay updated.  https://twitter.com/Intarocryptocurrency${NC}"
    echo -e "${BLUE}================================================================================================================================${NC}"
    echo -e "${CYAN}Ensure Node is fully SYNCED with BLOCKCHAIN before starting your Node :).${NC}"
    echo -e "${BLUE}================================================================================================================================${NC}"
    echo -e "${GREEN}Usage Commands.${NC}"
    echo -e "${GREEN}intarocryptocurrencyd masternode status${NC}"
    echo -e "${GREEN}intarocryptocurrencyd getinfo${NC}"
    echo -e "${BLUE}================================================================================================================================${NC}"
    echo -e "${NC}"
}
...
function setup_node() {
  get_ip
  update_config
  systemctl restart intarocryptocurrency
}

function install_intarocryptocurrency() {
    echo "You chose to install the Intarocryptocurrency Node"
    echo "Checking for Intarocryptocurrency installation"
    if [ -e /usr/bin/intarocryptocurrencyd ] || [ -e /usr/local/bin/intarocryptocurrencyd ]; then
        purgeOldInstallation
    else
    ...
    if [ "$compilefromsource" = "y" ] || [ "$compilefromsource" = "Y" ] ; then
        if [ -e ../Intarocryptocurrency.pro ]; then
            echo "Compiling Source Code"
            chmod +x build-unix.sh
            ./build-unix.sh
            mv ../bin/intarocryptocurrencyd /usr/bin
        else
            echo "Cloning github repository.."
            git clone https://github.com/intarocryptocurrency/intarocryptocurrency
            chmod +x ./intarocryptocurrency/scripts
            ./intarocryptocurrency/scripts/build-unix.sh
            mv ./intarocryptocurrency/bin/intarocryptocurrencyd /usr/bin
            if [ -e "$HOME"/intarocryptocurrency-swap ]; then
                echo "Removing temporary swap file"
                swapoff "$HOME"/intarocryptocurrency-swap
                rm "$HOME"/intarocryptocurrency-swap
            fi
        fi
    else
        ...
        mkdir ./tmp
        cd tmp
        wget https://github.com/$(wget https://github.com/intarocryptocurrency/intarocryptocurrency/releases/latest -O - | egrep '/.*/.*/.*tar.gz' -o)
        tar -xvf *.tar.gz
        mv ./intarocryptocurrencyd /usr/bin
        cd ..
        rm -r ./tmp
    fi
    create_config
...
function compile_linux_daemon() {
    echo "You chose to compile the Intarocryptocurrency CLI Daemon/Wallet"
    checks
    prepare_system
    if [ ! -e ../Intarocryptocurrency.pro ] ; then
        echo "Cloning Intarocryptocurrency Coin github repository to this directory."
        git clone https://github.com/intarocryptocurrency/intarocryptocurrency
        ./intarocryptocurrency/scripts/build-unix.sh
        clear
        echo "Compile is complete, you can find the binary file in ./intarocryptocurrency/bin/"
    else
        echo "Compiling Source Code"
        ./build-unix.sh
        clear
...
function compile_linux_gui() {
    ...
    prepare_system
    if [ ! -e ../Intarocryptocurrency.pro ] ; then
        echo "Cloning Intarocryptocurrency Coin Github Repository"
        git clone https://github.com/intarocryptocurrency/intarocryptocurrency
        ./intarocryptocurrency/scripts/build-unix.sh --with-gui
        clear
        echo "Compile is complete, you can find the binary file in ./intarocryptocurrency/bin/"
    else
        echo "Compiling Source Code"
...
function compile_windows_exe() {
    ...
    if [ ! -e ../Intarocryptocurrency.pro ] ; then
        ...
        git clone https://github.com/intarocryptocurrency/intarocryptocurrency
        chmod +x ./intarocryptocurrency/scripts/*
        ./intarocryptocurrency/scripts/clean.sh
        ./intarocryptocurrency/scripts/configure-mxe.sh
        ./intarocryptocurrency/scripts/build-win-mxe.sh
    else
        echo "Compiling Source Code"
        chmod +x ./*
        ./clean.sh
...
function setup_masternode() {
    ...
    if [ -e /usr/bin/intarocryptocurrencyd ] || [ -e /usr/local/bin/intarocryptocurrencyd ]; then
        read -p "There is already an installation of Intarocryptocurrency Coin. Did you want to use the currently installed software, or install the latest software? Y/n:" yn
        case $yn in
            [Yy]* ) install_intarocryptocurrency; setup_node; important_information;;
            [Nn]* ) backup_node_data; setup_node; important_information;;
            * ) echo "Sorry, did not understand your command, please enter Y/n";;
        esac
    else
        install_intarocryptocurrency
        setup_node
        important_information
    fi
}
...
function install_dependencies_only() {
    echo "You chose to install Intarocryptocurrency dependencies only"
    prepare_system
}
...
function backup_node_data() {
    ...
    today="$( date +"%Y%m%d" )"
    #Create a backups folder inside users home directory
    test -d ~/IntarocryptocurrencyBackups && echo "Backups folder exists" || mkdir ~/IntarocryptocurrencyBackups
    iteration=0
    while test -d "$BACKUP_FOLDER/$today$suffix"; do
        (( ++iteration ))
        suffix="$( printf -- '-%02d' "$iteration" )"
    ...
    cp $CONFIG_FOLDER/Intarocryptocurrency.conf $BACKUP_FOLDER/$foldername
    cp $CONFIG_FOLDER/wallet.dat $BACKUP_FOLDER/$foldername
}

function uninstall() {
    read -p "You chose to uninstall Intarocryptocurrency, would you like to continue? y/n:" yn
    case $yn in
...
echo "========================================================="
echo "[1] - Install Intarocryptocurrency node (will uninstall/upgrade existing installation)"
echo "[2] - Compile only (will compile linux daemon, linux GUI, or windows GUI - no install)"
echo "[3] - Prepare masternode (will install Intarocryptocurrency Node if needed)"
echo "[4] - Install dependencies only"
echo "[5] - Backup Intarocryptocurrency wallet and settings"
echo "[6] - Uninstall Intarocryptocurrency"

read choice1

case $choice1 in
    "1") install_intarocryptocurrency;;
    "2") compile_only;;
    "3") setup_masternode;;
