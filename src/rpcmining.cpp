...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Intarocryptocurrency is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Intarocryptocurrency is downloading blocks...");
...
Value getwork(const Array &params, bool fHelp) {
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Intarocryptocurrency is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Intarocryptocurrency is downloading blocks...");
