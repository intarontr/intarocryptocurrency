...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("Intarocryptocurrency"))
        return false;
...
{
    // Convert Intarocryptocurrency:// to Intarocryptocurrency:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Intarocryptocurrency://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "Intarocryptocurrency:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Intarocryptocurrency.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Intarocryptocurrency.lnk
...
{
    return GetAutostartDir() / "Intarocryptocurrency.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a Intarocryptocurrency.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Intarocryptocurrency\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Intarocryptocurrency app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Intarocryptocurrency app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Intarocryptocurrency-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Intarocryptocurrency-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Intarocryptocurrency-Qt"));
