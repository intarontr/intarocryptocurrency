...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeIntarocryptocurrencyAmount"))
        settings.setValue("nAnonymizeIntarocryptocurrencyAmount", 1000);
    nAnonymizeIntarocryptocurrencyAmount = settings.value("nAnonymizeIntarocryptocurrencyAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeIntarocryptocurrencyAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeIntarocryptocurrencyAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeIntarocryptocurrencyAmount:
            return QVariant(nAnonymizeIntarocryptocurrencyAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeIntarocryptocurrencyAmount:
            nAnonymizeIntarocryptocurrencyAmount = value.toInt();
            settings.setValue("nAnonymizeIntarocryptocurrencyAmount", nAnonymizeIntarocryptocurrencyAmount);
            emit AnonymizeIntarocryptocurrencyAmountChanged(nAnonymizeIntarocryptocurrencyAmount);
            break;
        default:
            break;
        }
