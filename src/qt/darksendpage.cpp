...
void DarksendPage::toggleDarksend()
{
    ...
    else 
    {
        ...
        if (nAnonymizeIntarocryptocurrencyAmount == 0)
        {
...
void DarksendPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeIntarocryptocurrencyAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeIntarocryptocurrencyAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        //strAnonymizeIntarocryptocurrencyAmount = strAnonymizeIntarocryptocurrencyAmount.remove(strAnonymizeIntarocryptocurrencyAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeIntarocryptocurrencyAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeIntarocryptocurrencyAmount*COIN) nMaxToAnonymize = nAnonymizeIntarocryptocurrencyAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeIntarocryptocurrencyAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeIntarocryptocurrencyAmount));
        //strAnonymizeIntarocryptocurrencyAmount = strAnonymizeIntarocryptocurrencyAmount.remove(strAnonymizeIntarocryptocurrencyAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeIntarocryptocurrencyAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
    ...
    QString strDenomBalance = QString().setNum(denomBalance, 'g', 4) + " NTR";
