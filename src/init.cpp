...
void Shutdown()
{
    ...
    RenameThread("Intarocryptocurrency-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Intarocryptocurrency.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Intarocryptocurrencyd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeintarocryptocurrencyamount=<n> " + _("Keep N Intarocryptocurrency anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Intarocryptocurrency is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Intarocryptocurrency is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Intarocryptocurrency version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Intarocryptocurrency server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Intarocryptocurrency") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Intarocryptocurrency to complete") << "\n";
    ...
    nAnonymizeIntarocryptocurrencyAmount = GetArg("-anonymizeintarocryptocurrencyamount", 0);
    if(nAnonymizeIntarocryptocurrencyAmount > 999999) nAnonymizeIntarocryptocurrencyAmount = 999999;
    if(nAnonymizeIntarocryptocurrencyAmount < 2) nAnonymizeIntarocryptocurrencyAmount = 2;
    ...
    LogPrintf("Anonymize Intarocryptocurrency Amount %d\n", nAnonymizeIntarocryptocurrencyAmount);
