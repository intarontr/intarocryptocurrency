...
int nAnonymizeIntarocryptocurrencyAmount = 1000;
...
#else
    const char* pszModule = "Intarocryptocurrency";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Intarocryptocurrency
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Intarocryptocurrency
    // Mac: ~/Library/Application Support/Intarocryptocurrency
    // Unix: ~/.Intarocryptocurrency
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Intarocryptocurrency";
#else
    fs::path pathRet;
    ...
    return pathRet / "Intarocryptocurrency";
#else
    // Unix
    return pathRet / ".Intarocryptocurrency";
#endif
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Intarocryptocurrency.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Intarocryptocurrencyd.pid"));
