...
//
// Bootup the masternode, look for a 3000 NTR input and register on the network
//
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if (status != MASTERNODE_REMOTELY_ENABLED)
        {
            ...
            if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
                ...
                /* rewards are not supported in Intarocryptocurrency.conf */
...
{
    CIntarocryptocurrencyAddress address(collateralAddress);
